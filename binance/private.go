...
// OrderMarket -
func (b *Binance) OrderMarket(symbol, side, quantity string) {
	timestamp := strconv.Itoa(int(unixMillis(time.Now())))
	parameters := fmt.Sprintf("symbol=%s&side=%s&type=MARKET&quantity=%s&recvWindow=5000&timestamp=%s", symbol, side, quantity, timestamp)
